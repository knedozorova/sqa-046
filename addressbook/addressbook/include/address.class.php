...
function saveAddress($addr_array, $group_id = "") {
    ...
    $sql = "INSERT INTO $table ( domain_id, firstname, middlename, lastname, nickname, company, title, address, home, mobile, work, fax, email, email2, email3, homepage, aday, amonth, ayear, bday, bmonth, byear, address2, phone2, photo, notes, created, modified)
    ...
    if(!isset($addr_array['id']) && $group_id) {
    	$sql = "INSERT INTO $table_grp_adr SELECT $domain_id domain_id, $id id, group_id, now(), now(), NULL FROM $table_groups WHERE group_id = '$group_id'";
    	$result = mysql_query($sql);
    }
...
function updateAddress($addr, $keep_photo = true) {
  ...
  global $keep_history, $domain_id, $base_from_where, $table, $table_grp_adr, $table_groups;
    ...
    $sql = "INSERT INTO $table ( domain_id, id, firstname, middlename, lastname, nickname, company, title, address, home, mobile, work, fax, email, email2, email3, homepage, aday, amonth, ayear, bday, bmonth, byear, address2, phone2, photo, notes, created, modified)
                        SELECT   $domain_id                                        domain_id
                               , '".getIfSetFromAddr($addr, 'id')."'  id
                               , '".getIfSetFromAddr($addr, 'firstname')."'  firstname
                               , '".getIfSetFromAddr($addr, 'middlename')."' lastname
                               , '".getIfSetFromAddr($addr, 'lastname')."'   lastname
                               , '".getIfSetFromAddr($addr, 'nickname')."'   nickname
                               , '".getIfSetFromAddr($addr, 'company')."'    company
                               , '".getIfSetFromAddr($addr, 'title')."'      title
                               , '".getIfSetFromAddr($addr, 'address')."'    address
                               , '".getIfSetFromAddr($addr, 'home')."'       home
                               , '".getIfSetFromAddr($addr, 'mobile')."'     mobile
                               , '".getIfSetFromAddr($addr, 'work')."'       work
                               , '".getIfSetFromAddr($addr, 'fax')."'        fax
                               , '".getIfSetFromAddr($addr, 'email')."'      email
                               , '".getIfSetFromAddr($addr, 'email2')."'     email2
                               , '".getIfSetFromAddr($addr, 'email3')."'     email3
                               , '".getIfSetFromAddr($addr, 'homepage')."'   homepage
                               , '".getIfSetFromAddr($addr, 'aday')."'       aday
                               , '".getIfSetFromAddr($addr, 'amonth')."'     amonth
                               , '".getIfSetFromAddr($addr, 'ayear')."'      ayear
                               , '".getIfSetFromAddr($addr, 'bday')."'       bday
                               , '".getIfSetFromAddr($addr, 'bmonth')."'     bmonth
                               , '".getIfSetFromAddr($addr, 'byear')."'      byear
                               , '".getIfSetFromAddr($addr, 'address2')."'   address2
                               , '".getIfSetFromAddr($addr, 'phone2')."'     phone2
                               , '".getIfSetFromAddr($addr, 'photo')."'      photo
                               , '".getIfSetFromAddr($addr, 'notes')."'      notes
                               , now(), now()
                            FROM DUAL";
    $result = mysql_query($sql);
