...
if(!$is_fix_group and isset($group_name))
{
...
if(!isset($group_name)) {
  $group_name = null;
  if (isset($group)) {
	$sql="SELECT group_name FROM $table_groups WHERE group_id=$group";
	$result_groups = mysql_query($sql);
	$result_groups_number = mysql_numrows($result_groups);
	if ($result_groups_number > 0) {
